...
- `com.company.prices.infraestructure.api.mappers`: Contains the mappers for converting domain entities to DTOs (e.g.,
  `PricesMapper`).
...
- `com.company.prices.infraestructure.persistence.mappers`: Contains the mappers for converting JPA to domain entities (
...
To generate the OpenAPI specification file using the Maven OpenAPI plugin, follow these steps:

1. Ensure your Spring Boot application is running.
2. Run the following Maven goal to generate the OpenAPI documentation:

```sh
mvn springdoc-openapi:generate
```

This will generate the `prices_api.json` file in the `src/main/resources/doc` directory.